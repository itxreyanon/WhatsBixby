...
function getTempFile(ext) {
    ensureTempDir();
    return path.join(TEMP_DIR, `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.${ext}`);
}

async function imageToWebp(media) {
    const tmpFileOut = getTempFile('webp');
    const tmpFileIn = getTempFile('jpg');
...
async function videoToWebp(media) {
    const tmpFileOut = getTempFile('webp');
    const tmpFileIn = getTempFile('mp4');
...
async function writeExifImg(media, metadata) {
    ...
    if (metadata.packname || metadata.author) {
        ...
        const json = {
            "sticker-pack-id": "https://github.com/c-o-d-e-xx/AstraWa",
